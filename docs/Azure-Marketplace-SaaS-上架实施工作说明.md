# Azure Marketplace（Microsoft Commercial Marketplace）SaaS 上架实施工作说明（基于 SaaS Accelerator 抽象）

> 目标：把“要上架一个 SaaS 应用到 Azure Marketplace”拆解成可执行的工作项、接口清单、数据模型与验收标准，方便团队照着实现并评估工作量。
>
> 适用：不限定技术栈（.NET / Java / Node.js / Python / Go / 任意）。本文以本仓库的 **SaaS Accelerator** 作为参考实现来源，抽象出通用模式。
...
```mermaid
architecture-beta
  group marketplace(cloud)[Microsoft Commercial Marketplace]
    service fulfill_api(internet)[SaaS Fulfillment API]
    service meter_api(internet)[Marketplace Metering]
  
  group publisher(cloud)[Publisher Cloud / Your SaaS]
    group web(cloud)[Web Layer]
      service landing(server)[Landing Page / Customer Portal]
      service webhook(server)[Webhook Endpoint]
      service admin(server)[Admin Portal (Optional)]
    
    group core(cloud)[Core Services]
      service saas_service(server)[SaaS Provisioning Service]
      service metering(server)[Usage Aggregator / Metering Service]
      service scheduler(server)[Scheduler / Jobs (Optional)]

    service db(database)[Subscription & Audit DB]
    service kv(disk)[Secrets (Key Vault / KMS)]
    service mon(server)[Monitoring & Logs]

  fulfill_api:R --> L:landing
  landing:R --> L:saas_service
  saas_service:R --> L:db
  saas_service:R --> L:kv
  webhook:R --> L:saas_service
  saas_service:R --> L:fulfill_api
  metering:R --> L:meter_api
  scheduler:R --> L:metering
  landing:R --> L:mon
  webhook:R --> L:mon
  admin:R --> L:mon
```

> 如果你的 Mermaid 渲染器不支持 `architecture-beta`，可以把上图替换成 `flowchart` 版本；不影响本文步骤。
...
**输入**：Marketplace 重定向到你的 Landing Page，并在 URL 中携带 `marketplace token`（以及你自定义的一些参数）。
...
sequenceDiagram
  ...
  participant SF as SaaS Fulfillment API
  participant SaaS as Your SaaS Backend
  participant DB as Subscription DB

  Buyer->>MP: Purchase SaaS Offer/Plan
  MP->>LP: Redirect (marketplaceToken)
  LP->>SF: Resolve(marketplaceToken)
  SF-->>LP: subscriptionId, offerId, planId, purchaser info
  LP->>SaaS: Provision request (subscriptionId + customer inputs)
  SaaS->>DB: Upsert subscription mapping + status
  SaaS-->>LP: Provision result
  alt Auto activation
    LP->>SF: Activate(subscriptionId, planId)
    SF-->>LP: Activated
    LP-->>Buyer: Show success / enter app
  else Pending activation
    LP-->>Buyer: Show "Pending activation" + next steps
  end
```
...
sequenceDiagram
  ...
  MP->>WH: Notify change (plan/quantity)
  WH->>SaaS: Validate + enqueue event
  SaaS->>SF: Get operation/status (poll)
  SF-->>SaaS: Operation status
  alt Accepted
    SaaS->>DB: Update subscription + apply new entitlements
    SaaS-->>MP: Acknowledge/complete (per API)
  else Rejected
    SaaS->>DB: Record rejection reason
    SaaS-->>MP: Reject update (per API)
  end
```
...
sequenceDiagram
  ...
  participant DB as Usage/Audit DB
  participant Meter as Marketplace Metering
